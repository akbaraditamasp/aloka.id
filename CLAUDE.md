...
**Product/brand context:** read `PRODUCT.md` at the project root for anything about what this site is, who it's for, its business model, sitemap, or brand/content details (Aloka). This file only covers the framework itself.

**Visual style:** read `STYLE.md` at the project root before writing or editing any `.edge` template, `main.ts`, or `app.css` — it defines Aloka's color tokens, typography, spacing/component conventions, and the mandatory rules (Tailwind tokens only, no arbitrary values without justification; icons via Iconify).
